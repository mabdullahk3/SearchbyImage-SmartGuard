...
export default function SearchByImagePage() {
  ...
  const [errorMessage, setErrorMessage] = useState(null) // State to handle the error message
  ...
  const handleImageUpload = async (event) => {
    ...
    if (file) {
      ...
      try {
        ...
        if (!data || !data.name || !data.date || !data.images) {
  ...
  return (
    <div className="min-h-screen bg-[white] text-white pt-2 px-8 relative">
      ...
      <div className="bg-[#9f24c2] p-6 rounded-lg mb-8">
        <label htmlFor="image-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-white border-dashed rounded-lg cursor-pointer hover:bg-[#7f35b6]">
